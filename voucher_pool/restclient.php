...
//     	$headers = ['Content-Type'=>['application/json']];
        $default_options = [
            ...
            'parameters' => [], 
            'curl_options' => [], 
            'user_agent' => "EMR to Connect MongoDB Via REST", 
            'base_url' => 'http://localhost/slim_app/public/', 
            'format' => '', 
            'format_regex' => "/(\w+)\/(\w+)(;[.+])?/",
            'decoders' => [
                'json' => 'json_decode', 
    ...
    public function execute($url, $method='GET', $parameters=[], $headers=[]){
       ...
       if($client->options['auth'])
       		$curlopt[CURLOPT_HTTPAUTH] =  constant ( 'CURLAUTH_' . strtoupper ( $client->options['auth'] ) );
